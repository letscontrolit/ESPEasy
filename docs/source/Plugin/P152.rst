...
* **DAC1 / GPIO-17**: First available DAC output pin. If PSRAM is available on the ESP unit, then this pin is often used to control the PSRAM chip, and the selector shows that as ``[PSRAM]``. It is *strongly* advised not to use the pin as a DAC output if PSRAM is installed, and certainly not if it is in use.
