...
* **Export only enabled tasks**: Export only the tasks that are enabled, limiting the amount of data stored and sent.

Included in the Github repository is an example Python script to decode the binary format. This can be found in the ``misc``
...
The values will be restored from RTC memory on reboot or deep sleep, so the unit can continue to upload data when re-connected.

However the values are lost on a power cycle, so some data may be sent multiple times if the ESP is power cycled before a file was finished. (or no bin file will be deleted after upload).
