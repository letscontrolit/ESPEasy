...
By the physical design of these modules, using NeoPixel leds for display, the power consumption is dependent on the number of neopixels that are activated and the brightness that is set. On a display consisting of 4 matrices, the power used was ca. 3A @ 5V with all leds on at brightness 255, so it requires an adequate power supply, preferably separate from the power supply used to power the ESP.
...
* **Clear display on exit** When checked, will clear the display when the task is disabled, either from settings or via the ``TaskDisable`` command. This will fill the display with all black pixels, turning all NeoPixels off.
