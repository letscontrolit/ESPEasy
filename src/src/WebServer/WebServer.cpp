...
void getWebPageTemplateDefaultHeader(WebTemplateParser& parser, const __FlashStringHelper * title, bool addMenu) {
  {
    ...
    parser.process(F("<header class='headermenu'><h1>ESP Easy Mega <b>"));
    parser.process(title);
    parser.process(F("</b>"));
    #if BUILD_IN_WEBHEADER
    parser.process(F("<div class='build'>Build: " GITHUB_RELEASES_LINK_PREFIX "{{date}}" GITHUB_RELEASES_LINK_SUFFIX "</div>"));
