          ...
          for (taskIndex_t x = 0; x < TASKS_MAX; x++) {
            ...
            if (Settings.TaskDeviceEnabled[x] &&
                (Settings.TaskDeviceSendData[ControllerID][x]
                 || (Settings.getPluginID_for_task(x) == pluginID_t::toPluginID(29))         // Domoticz helper doesn't have controller checkboxes...
                 # if defined(USES_P088)
                 || (Settings.getPluginID_for_task(x) == pluginID_t::toPluginID(88))         // Heatpump IR doesn't have controller checkboxes...
