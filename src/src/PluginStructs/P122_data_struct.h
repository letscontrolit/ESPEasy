...
public:
  ...
  bool setupDevice(uint8_t i2caddr, uint8_t resolution);
...
protected:
  ...
  uint16_t  _rawHumidity;       // Humidity as fetched from the device [bits]
  uint16_t  _rawTemperature;    // Temperature as fetched from the device [bits]
  uint8_t   _resolution;        // Programmed resolution
