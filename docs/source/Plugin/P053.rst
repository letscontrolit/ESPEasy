...
- PM1.0 - Concentration of particles > 1 micron (µm/m3)
- PM2.5 - Concentration of particles > 2.5 micron (µm/m3)
- PM10 - Concentration of particles > 10 micron (µm/m3)

*(As shown in the screenshot above)*

*Particles µg/m3: pm2.5; Other: Temp, Humi, HCHO (PMS5003ST)* Output values, providing some measurements only supported by the PMS5003ST model.
...
*Part. count/0.1L: cnt1.0, cnt2.5, cnt5, cnt10 (PMS1003/5003(ST)/7003)* Count of Particles per 0.1 L of air, available on the PMS1003, PMS5003, PMS5003ST and PMS7003 sensor models.

- cnt1.0 - Count of particles per 0.1L of air > 1 micron
- cnt2.5 - Count of particles per 0.1L of air > 2.5 micron
- cnt5 - Count of particles per 0.1L of air > 5 micron
- cnt10 - Count of particles per 0.1L of air > 10 micron
...
**Events for non-output values** When an option other than **None** is selected, the **Output values** that are not directly available, will be sent via events. The names of these events are:
...
*Particles µg/m3, Temp/Humi/HCHO and Particle count / 0.1L*:

All of the above, and also ``<Taskname>#cnt1.0``, ``<Taskname>#cnt2.5`` and ``<Taskname>#cnt10`` events are generated if these are not selected as **Output values**.
...
Example: (Sensor device called PMS5003ST and Dummy device named ``Particles`` with 2 values named ``pm1.0`` and ``pm10``)
...
These rules use a ``System#Boot`` event to set GPIO16 to low (0) and start a 20 sec loop-timer. (Another timer can be used if desired, then also adjust the Rules#Timer=n value)

The ``Rules#Timer=1`` event is triggered by this timer and sends a 5 seconds "high" pulse to the sensor waking it up.

Set the delay inside the dustsensor task to 30 seconds for this. The ESPEasy plugin gets the data and stores it when the sensor gets active.
