...
private:
  ...
  void send(uint8_t improvType, const std::vector<uint8_t> &response);
...
public:
   ...
   *
   */
  bool handleSerial(uint8_t b, Stream *serialForWrite);
   ...
   *
   */
  void setDeviceInfo(const char *firmwareName, const char *firmwareVersion, const char *deviceName, const char *deviceUrl);
  void setDeviceInfo(const char *firmwareName, const char *firmwareVersion, const char *deviceName);
