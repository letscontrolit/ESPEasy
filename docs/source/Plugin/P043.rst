...
* **Value input On/Off only**: When checked, the selection for Value will be for On/Off (or empty) only, to avoid misconfiguration for less experienced users. The default is unchecked.

.. image:: P043_GPIO_Enabled.png

* **Day,Time X**: Select the Day and Time that should be checked. For Limited builds the day and time have to be entered manually, for other builds, tha Day can be selected from a combobox, and the time can be selected from a predefined list and set to a desired time.
