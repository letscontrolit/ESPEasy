...
int                     P003_currentStableState[P003_TASKS_MAX];     // stores current stable pin state. Set in Step 3 when new stable pulse started
...
unsigned int P003_Step2OKcounter[P003_TASKS_MAX];           // counts how often step 2 detected the expected pin state (first verification)
unsigned int P003_Step2NOKcounter[P003_TASKS_MAX];          // counts how often step 2 detected the wrong pin state (first verification failed)
unsigned int P003_Step3OKcounter[P003_TASKS_MAX];           // counts how often step 3 detected the expected pin state (2nd verification)
unsigned int P003_Step3NOKcounter[P003_TASKS_MAX];          // counts how often step 3 detected the wrong pin state (2nd verification failed)
unsigned int P003_Step3IGNcounter[P003_TASKS_MAX];          // counts how often step 3 detected the wrong pin state (2nd verification failed)
...
byte P003_StatsLogLevel[P003_TASKS_MAX];                    // log level for regular statistics logging (default = P003_PULSE_STATS_ADHOC_LOG_LEVEL)
  ...
  switch (pStep)
  {
    ...
    // regularily called to check if the trigger has flagged the next signal edge
    {
      if( bitRead(P003_initStepsFlags, taskIndex) )
      {
        ...
        long delayTime = static_cast<long>((static_cast<int64_t>(P003_debounceTime[taskIndex]))-static_cast<int64_t>(getMicros64()-P003_triggerTimestamp[taskIndex]))/1000L;
        if(delayTime < 0)
        {              
