...
#define PLUGIN_NAME_078       "Input - iButton"
#define PLUGIN_VALUENAME1_078 "iButton"

int8_t Plugin_078_DallasPin;
    ...
    switch (function)
    {
        ...
        case PLUGIN_WEBFORM_LOAD:
        {
            ...
            if (Plugin_078_DallasPin != -1){
              ...
              addSelector_Head(F("plugin_078_dev"), false);
              addSelector_Item("", -1, false, false, F(""));
              uint8_t tmpAddress[8];
              byte count = 0;
              Plugin_078_DS_reset();
              Plugin_078_DS_reset_search();
              while (Plugin_078_DS_search(tmpAddress))
              {
                  String option = "";
                  for (byte j = 0; j < 8; j++)
                  {
        ...
        case PLUGIN_TEN_PER_SECOND: //PLUGIN_READ:
        {
            if (ExtraTaskSettings.TaskDevicePluginConfigLong[0] != 0){
                ...
                if (Plugin_078_DS_readiButton(addr))
                {
...
void Plugin_078_get_addr(uint8_t addr[], byte TaskIndex)
{
...
/*********************************************************************************************\
   Dallas Scan bus
  \*********************************************************************************************/
byte Plugin_078_DS_scan(byte getDeviceROM, uint8_t* ROM)
{
    byte tmpaddr[8];
    byte devCount = 0;
    Plugin_078_DS_reset();
...
uint8_t Plugin_078_DS_reset()
{
    ...
    uint8_t retries = 125;
      ...
      ESP32noInterrupts();
    #endif
    pinMode(Plugin_078_DallasPin, INPUT);
    ...
    while (!digitalRead(Plugin_078_DallasPin));

    pinMode(Plugin_078_DallasPin, OUTPUT); digitalWrite(Plugin_078_DallasPin, LOW);
    delayMicroseconds(492);               // Dallas spec. = Min. 480uSec. Arduino 500uSec.
    pinMode(Plugin_078_DallasPin, INPUT); // Float
    delayMicroseconds(40);
    r = !digitalRead(Plugin_078_DallasPin);
...
*  Dallas Search bus
\*********************************************************************************************/
uint8_t Plugin_078_DS_search(uint8_t * newAddr)
{
    ...
    if (!LastDeviceFlg)
    {
        ...
        Plugin_078_DS_write(0xF0);

        // loop to do the search
        do
        {
            // read a bit and its complement
            id_bit     = Plugin_078_DS_read_bit();
            cmp_id_bit = Plugin_078_DS_read_bit();
            ...
            else
            {
                ...
                Plugin_078_DS_write_bit(search_direction);
...
*  Dallas Read byte
\*********************************************************************************************/
uint8_t Plugin_078_DS_read(void)
{
    uint8_t bitMask;
    uint8_t r = 0;

    for (bitMask = 0x01; bitMask; bitMask <<= 1)
        if (Plugin_078_DS_read_bit())
            r |= bitMask;
...
*  Dallas Write byte
\*********************************************************************************************/
void Plugin_078_DS_write(uint8_t ByteToWrite)
{
    uint8_t bitMask;
    for (bitMask = 0x01; bitMask; bitMask <<= 1)
        Plugin_078_DS_write_bit( (bitMask & ByteToWrite) ? 1 : 0);
}

/*********************************************************************************************\
*  Dallas Read bit
\*********************************************************************************************/
uint8_t Plugin_078_DS_read_bit(void)
{
    uint8_t r;
       ...
       ESP32noInterrupts();
    #endif
    pinMode(Plugin_078_DallasPin, OUTPUT);
    digitalWrite(Plugin_078_DallasPin, LOW);
    delayMicroseconds(3);
    pinMode(Plugin_078_DallasPin, INPUT); // let pin float, pull up will raise
...
boolean Plugin_078_DS_readiButton(byte addr[8])
{
    ...
    Plugin_078_DS_write(0x55); // Choose ROM
    for (byte i = 0; i < 8; i++)
        Plugin_078_DS_write(addr[i]);

    Plugin_078_DS_write(0xBE); // Read scratchpad

    for (byte i = 0; i < 9; i++) // read 9 bytes
        ScratchPad[i] = Plugin_078_DS_read();
...
*  Dallas Write bit
\*********************************************************************************************/
void Plugin_078_DS_write_bit(uint8_t v)
{
    if (v & 1)
    {
        #if defined(ESP32)
        ...
        pinMode(Plugin_078_DallasPin, OUTPUT);
        delayMicroseconds(10);
        digitalWrite(Plugin_078_DallasPin, HIGH);
        #if defined(ESP32)
          ESP32interrupts();
        #endif
...
\*********************************************************************************************/
boolean Plugin_078_DS_crc8(uint8_t * addr)
{
  uint8_t crc = 0;
  uint8_t len = 8;
