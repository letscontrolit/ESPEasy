...
#define PLUGIN_NAME_082       "OpenTherm boiler device"
#define PLUGIN_VALUENAME1_082 "OpenTherm"
#define P33_Nlines 4
#define P33_Nchars 8
...
boolean Plugin_082(byte function, struct EventStruct *event, String& string)
{
  ...
  switch (function)
  {
    ...
    case PLUGIN_WEBFORM_LOAD:
      {
        ...
        addFormSelector(F("Simulate Data Type"), F("p082_sensortype"), 11, options, optionValues, choice );
        ...
        addCheckBox(F("p082_keepvalue"), keepValue);

        char deviceTemplate[P33_Nlines][P33_Nchars];
        LoadCustomTaskSettings(event->TaskIndex, (byte*)&deviceTemplate, sizeof(deviceTemplate));
        for (byte varNr = 0; varNr < P33_Nlines; varNr++)
        {
          addFormTextBox(String(F("Persistent value ")) + (varNr + 1), String(F("p082_template")) + (varNr + 1), deviceTemplate[varNr], P33_Nchars);
        }
        success = true;
        break;
      }
    ...
    case PLUGIN_WEBFORM_SAVE:
      {
        Settings.TaskDevicePluginConfig[event->TaskIndex][0] = getFormItemInt(F("p082_sensortype"));
        Settings.TaskDevicePluginConfig[event->TaskIndex][1] = isFormItemChecked(F("p082_keepvalue"));
