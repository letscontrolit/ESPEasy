...
*Normal* For measurements in the 0 to 1300 millimeter (130cm) range.

*Long* For measurements in the 0 to 4000 millimeter (400cm) range (but somewhat less accurate).

**Send event when value unchanged** To avoid many of the same events when the measurement is stable, this option is off by default. When enabled, every measurement will cause an event, and send the data to any enabled Controllers.

The Data Acquisition, Send to Controller and Interval settings are standard available configuration items. Send to Controller only when one or more Controllers are configured.

**Interval** By default, Interval will be set to 0, meaning that once a measurement is completed, it will be read. This implies that quite a lot of events will be generated, possibly putting quite some load on the ESP. Setting this to 1 or 2 seconds, usually offers a reasonable response time.
...
The Ambient ligthing condition during measurement is available in ``Ambient``. The unit is kcps (Photons per second, recalculated to kilo count per second)
...
.. versionchanged:: 2.0
  ...
  |added| 2021-04-05 Added to main repository Plugin 113, based on a copy of Plugin 110 Distance - VL53L0X
