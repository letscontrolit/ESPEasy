...
This can be set to a server url (https preferred, they are downloaded by trhe browser, **not** by the ESPEasy unit) that hosts *all* files as listed on :ref:`ExternalHostedStaticFiles`
...
For users that want to host these files in their local network, or if the files on jdeliver.net have expired, an alternative location for these .js and .css file (optionally gzipped into .gz files) can be configured.

Setting this location will **override** the default CDN url, except for Custom builds that already *have* the CDN url set, then this setting won't be available!
