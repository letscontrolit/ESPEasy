...
*Particles /m3: pm1.0, pm2.5, pm10* The default output values, available for all supported sensors.

- PM1.0 - Concentration of particles > 1 µm/m3
- PM2.5 - Concentration of particles > 2.5 µm/m3
- PM10 - Concentration of particles > 10 µm/m3

*(As shown in the overview screenshot above)*

*Particles /m3: pm2.5; Other: Temp, Humi, HCHO (PMS5003ST)* Particles 2.5 µm/m3 combined with other measurement values only supported by the PMS5003ST sensor.
...
*Particles /0.1L: cnt1.0, cnt2.5, cnt5, cnt10 (PMS1003/5003(ST)/7003)* Count of Particles per 0.1 L of air, available on the PMS1003, PMS5003, PMS5003ST and PMS7003 sensor models.
...
*Particles /m3 and Temp/Humi/HCHO*:

When *Particles /m3: pm1.0, pm2.5, pm10* is selected: ``<Taskname>#Temp``, ``<Taskname>#Humi`` and ``<Taskname>#HCHO``, with the measured value available in ``%eventvalue1%`` (or it's alias ``%eventvalue%``).

When *Particles /m3: pm2.5; Other: Temp, Humi, HCHO (PMS5003ST)* is selected: ``<Taskname>#pm1.0`` and ``<Taskname>#pm10``.

*Particles /m3, Temp/Humi/HCHO and Particles /0.1L*:
