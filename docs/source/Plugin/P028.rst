...
* **Error State Value** If no sensor is connected, or the sensor suffers from read errors for some reason, the **Temperature** value to report in that situation can be set here. Both **Humidity** and **Pressure** will be set to -1 when read errors occur.

.. image:: P028_ErrorStateValueOptions.png
  :alt: Error State Value options

* *Ignore* Do not return any value (does log the error once per second at INFO level).
* *-127* Set to the out-of-range value of -127.
* *0* Set to the unknown value of 0.
* *125* Set to the out-of-range value of 125.
