...
The 74HC165 Shift registers can be used to connect many input pins to an ESP using only 4 GPIO pins, as these shift registers can easily be daisy chained.
...
* **GPIO -> Data pin (Q7)**: Select the GPIO pin that is connected to the Data out pin of the (first) shift register. Any daisy chained register has its ``DS`` pin connected to the ``Q7`` pin of the previous shift register.

* **GPIO -> Clock pin (CP)**: Select the Clock pin of all shift registers.

* **GPIO -> Enable pin (EN)**: Select the Enable pin of all shift registers.

* **GPIO -> Load pin (PL)**: Select the Load pin of all shift registers.

NB: All 4 pins need to be configured or the plugin will not start.
...
The Values ``State_A`` to ``State_D`` are used to show the state of up to 16 shift registers. (Custom formatted)
