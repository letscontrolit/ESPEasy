...
Circular scroll options
~~~~~~~~~~~~~~~~~~~~~~~

(Section is collapsed by default when Circular scroll is not enabled)

* **Circular scroll**: If the **Scroll text > display width** option is enabled, and also this checkbox is checked, the content set with ``7dtext`` or ``7dbin`` commands will be scrolled continuously (circular), with an optional separator.

* **Separator character**: Choose the desired separator character, space, ``-``, ``/``, ``|`` or ``\``. Default is a space.

* **Nr. of separator characters**: Choose from 0 to 15 separator characters, default 1.

* **Wrap separator with spaces**: When checked, the number of separator characters is wrapped with a space before and after. Example when enabled, content is 'some text' and the separator is a dash: ``some text - some text``
