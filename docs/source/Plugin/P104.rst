...
The MAX7219 LED controllers are versatile in the fact that they can control either a set of 7-segment displays (see :ref:`P073_Page`) or a matrix of up to 64 LEDs, where this plugin comes in. *(Technically, the MAX7221 chip is also supported by the MD_Parola/MD_MAX72XX libraries, but as I didn't find any boards using that chip, I can't officially say it is supported.)*
...
If multiple modules are to be connected daisy-chained, they all need to be of the same hardware type, as the Parola library doesn't support a different hardware type per zone.
...
**Year uses 4 digits** When enabled, the Date yy and Date/time content types use a 4 digit year display instead of 2 digits.
...
**Size** The number of modules that are in this zone. The maximum that can be set here is 255.

**Text** If a pre-defined text should be displayed for the zone, it can be set here. The usual variables and functions, as shown on the Tools/System Variables can be used, as well as the ``[Task#<variable>]`` and ``[Var#<n>]`` and ``%v<n>%`` variables. The system variables that produce special characters, like ``{D}`` etc., actually generate Unicode characters, and that is not supported by the, ASCII based, fonts used. To use any special characters look them up in the font tables, below, to have the desired characters displayed. This field is also used by the Bar graph content type for any predefined bar graph display. Maximum length of this field is 100 characters. If longer contents needs to be set, it can be set from the rules using the ``DotMatrix,<zone>,[set]txt,"<text content>"`` command. (``txt`` and ``settxt`` subcommands).
...
* *Date yy* Show the date in a day, month and year format, formatted as configured in the **Content options** settings.
...
**Animation In** The animation that is used to put the content into the zone. Default is Print, that just puts the content according to the alignment, in the zone. None is not available for **Animation In**, as it would not display anything.

**Animation Out** The animation used to remove the content from the zone. If set to None (the default), the content will stay in the zone until it is changed, or updated using the **Repeat (sec)** option.
...
Some animations are marked with an asterisk ``*`` as a warning they should not be combined with a **Special Effects** setting that is *also* marked with an asterisk.
...
On 'bin-size-challenged' builds the *Full, double height* font is replaced by the *Numeric, double height* font, to limit the bin-size a bit more. This font can display only numeric content.

**TODO:** *Draw font tables for all available fonts*
...
**Repeat (sec)** Allows to repeat the current content of a zone to be sent again, using the animations as configured. Can be set in seconds accuracy. -1 indicates that repeat is disabled for this zone.

This will also repeat any content (usually *Text* or *Bar graph*) that is set using the corresponding commands (see below), or refresh a *Bar graph* on a regular interval. The initially set content is re-evaluated before re-display.

**Action** These actions are related to changing the configuration of zones, not with the display of the zones.
...
* *None* No action.
* *Add above* Add a new Zone above the current zone.
* *Add below* Add a new Zone below the current zone.
* *Delete* Remove the current zone from the list. **This can not be undone!**

Above and Below are relative to the current Zone order, so if Zone order is in Numeric order, selecting Add above will shift all Zones 1 position down, and insert a new Zone above it, and if Zone order is in Display order, then action Add above will insert a new Zone 2 shifting any other zones 1 number up, and leave Zone 1 as is.

The *Add* options are only available if the maximum nr. of zones isn't reached yet.
