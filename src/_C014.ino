...
# endif // ifdef CPLUGIN_014_V3
        ...
        CPlugin_014_sendMQTTnode(nodename, F(CPLUGIN_014_SYSTEM_DEVICE), F("$name"), "",
                                 F(CPLUGIN_014_SYSTEM_DEVICE),  errorCounter);

        // $name	Device → Controller	Friendly name of the property.	Any String	Yes	No ("")
        CPlugin_014_sendMQTTnode(nodename, F(CPLUGIN_014_SYSTEM_DEVICE), F(CPLUGIN_014_CMD_VALUE), F("/$name"),
                                 F(CPLUGIN_014_CMD_VALUE_NAME), errorCounter);
                  ...
                  for (byte varNr = 0; varNr < valueCount; varNr++)
                  {
                    if (validPluginID_fullcheck(Settings.TaskDeviceNumber[x]))
                    {
                      if (ExtraTaskSettings.TaskDeviceValueNames[varNr][0] != 0) // do not send if Value Name is empty!
                      {
                        ...
                        // $datatype	The data type. See Payloads.	Enum: [integer, float, boolean,string, enum, color]
                        CPlugin_014_sendMQTTnode(nodename, deviceName, ExtraTaskSettings.TaskDeviceValueNames[varNr], F("/$datatype"),
                                                 F("float"), errorCounter);

                        if (Device[DeviceIndex].Number == 33) { // Dummy Device can send AND receive Data
                          CPlugin_014_sendMQTTnode(nodename, deviceName, ExtraTaskSettings.TaskDeviceValueNames[varNr], F("/$settable"),
                                                   F("true"), errorCounter);
                        }
                           ...
                           (still case sensitive)
                                                  ...
                                                  if (unitName != "")  // found a unit match
                                                  {
                                                    ...
                                                    CPlugin_014_sendMQTTnode(nodename, deviceName,
                                                       ExtraTaskSettings.TaskDeviceValueNames[varNr], F("/$unit"), unitName, errorCounter);
                                                  }
                                                  unitName = "";
                         */
                      }
      ...
      if (string != "") {
        ...
        } else // not gpio
        {
          ...
          if (validTaskVarIndex(taskVarIndex)) {
            ...
            {
              valueStr = toString(UserVar[userVarIndex], ExtraTaskSettings.TaskDeviceValueDecimals[taskVarIndex]); // parseString(string,
                                                                                                                   // 4);
