...
The SCD40 and SDC41 are CO2 sensor units that also provide Humidity and Temperature measurement, support auto-calibration, and has temperature and altitude compensation settings. Using low-power operation the measurements will be available slower, but average power usage will be much lower.

The SDC41 also supports single-shot measurements, to enable really low power consumption as it will enter a power-saving state when not measuring.
...
* **Device model**: Select the sensor model used, available options:

.. image:: P135_SensorModelOptions.png
  :alt: Sensor model options

* *SDC40* The default, 'basic', sensor model, that does not support Single-shot measurements (might show values when configured, but the values are undefined).

* *SCD41* The 'advanced' model that supports Single-shot measurements.
...
* **Single-shot measurements (SCD41 only)**: (Not shown when **Sensor model** SCD40 is selected) This option enables the use of Single-shot measurements, that will be started on every **Interval** trigger, and provide a result 5 seconds later. By using this option with a long Interval the power consumption can be really low, as it does not continuously perform measurements, but puts the chip in a power-saving mode until the next measurement. This option only works when having a SCD41 sensor connected, when enabled on an SCD40 the values are undefined, even though they seem somewhat reasonable.

* **Automatic Self Calibration**: When enabled, the automatic self-calibration will be activated when the plugin is started. If the calibration is reset via the ``scd4x,factoryreset`` command, the calibration is reset to factory settings, and calibration data will have to be built up again.
...
Get Config Values retrieve values or settings from the sensor or plugin, and can be used in Rules, Display plugins, Formula's etc. The square brackets **are** part of the variable. Replace ``<taskname>`` by the **Name** of the task.
