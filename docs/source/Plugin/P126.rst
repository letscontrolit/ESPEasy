...
The register outputs are **not** updated during initialization of the plugin; only when changing a pin state, or using the ``74hc595Update`` command, the shift register outputs are updated.
...
NB: The State values are grouped per 4 chips. To be read from right to left in chip-order. If less chips are configured, only available states are shown. Display is started at the **Offset for display** + 1 chip.
