...
* **Animation In** The animation that is used to put the content into the zone. Default is Print, that just puts the content according to the alignment, in the zone. None is not available for **Animation In**, as it would never display anything.
...
Some animations are marked with an asterisk ``*`` as a warning that they should not be combined with a **Special Effects** setting that is *also* marked with an asterisk. The result is undefined, and may give a distorted display.
...
* **Action** These actions are related to changing the configuration of zones, and not with the display of the zones.
