...
static const char jsFetchAndParseLog[] PROGMEM = {
  "<script>"
  "(function(){"
  "var FetchingText = 'Fetching log entries...';"
  "document.getElementById('copyText_1').innerHTML = FetchingText;"
  "var timeForNext = 1000;"
  "var c;"
  "var i = setInterval(function(){"
  "var url = '/logjson'; "
  ...
  "   try { logEntry = data.Log.Entries[c].timestamp;  } catch(err) { logEntry = err.name; }"
  "   finally { if (logEntry !== \"TypeError\") { if (document.getElementById('copyText_1').innerHTML == FetchingText) { document.getElementById('copyText_1').innerHTML = '';}"
  "  document.getElementById('copyText_1').innerHTML += '<div class=level_' + data.Log.Entries[c].level + ' id=' + data.Log.Entries[c].timestamp + '>"
  "<font color=\"gray\">' + data.Log.Entries[c].timestamp + ':</font> ' + data.Log.Entries[c].text + '</div>';"
  "  autoscroll_on = document.getElementById('autoscroll').checked;"
  " if (autoscroll_on == true) {"
  "  document.getElementById(data.Log.Entries[c].timestamp).scrollIntoView({behavior: \"smooth\"});}"
  "  timeForNext = data.Log.TTL;"
  " } else {timeForNext = 1000;  }}}});})"
  " .catch(function(err) {document.getElementById('copyText_1').innerHTML += '<div>>> ' + err.message + ' <<</div>'; });}, timeForNext);})();"
  "window.onblur = function() { window.blurred = true; }; window.onfocus = function() { window.blurred = false; }</script>"
  "<body onblur = \"function() { window.blurred = true; }\" onfocus = \"function() { window.blurred = false; }\">" 
};
