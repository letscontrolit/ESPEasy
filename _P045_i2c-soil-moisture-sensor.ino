//#######################################################################################################
//#################### Plugin 045 Moisture & Temperature & Light I2C Soil moisture sensor  ##############
//#######################################################################################################
//
...
#define PLUGIN_NAME_045       "Moisture & Temperature & Light I2C Soil moisture sensor [TEST]"
#define PLUGIN_VALUENAME1_045 "Temperature"
#define PLUGIN_VALUENAME2_045 "Moisture"
#define PLUGIN_VALUENAME3_045 "Light"
...
boolean Plugin_045_init[2] = {false, false};
...
boolean Plugin_045(byte function, struct EventStruct *event, String& string)
{
  ...
  switch (function)
  {
    ...
    case PLUGIN_GET_DEVICEVALUENAMES:
      {
        ...
        strcpy_P(ExtraTaskSettings.TaskDeviceValueNames[1], PSTR(PLUGIN_VALUENAME2_045));
        strcpy_P(ExtraTaskSettings.TaskDeviceValueNames[2], PSTR(PLUGIN_VALUENAME3_045));
        break;
      }
    ...
    case PLUGIN_WEBFORM_LOAD:
      {
        ...
        string += F("plugin_045_i2cSoilMoisture_i2cAddress' value='");
    ...
    case PLUGIN_WEBFORM_SAVE:
      {
        String plugin1 = WebServer.arg("plugin_045_i2cSoilMoisture_i2cAddress");
    ...
    case PLUGIN_READ:
      {
        ...
        Plugin_045_write8(SOILMOISTURESENSOR_MEASURE_LIGHT,SOILMOISTURESENSOR_MEASURE_LIGHT);

        UserVar[event->BaseVarIndex] = ((float)Plugin_045_readTemperature(idx)) / 10;
        UserVar[event->BaseVarIndex + 1] = ((float)Plugin_045_readMoisture(idx));

        delay(1000);  //we need this delay, otherwise we get only the last reading...
        UserVar[event->BaseVarIndex + 2] = ((float)Plugin_045_readLight(idx));
        // send sensor to sleep
        Plugin_045_write8(SOILMOISTURESENSOR_SLEEP, SOILMOISTURESENSOR_SLEEP);
        ...
        if (UserVar[event->BaseVarIndex]>100 || UserVar[event->BaseVarIndex] < -40 ||
          ...
          UserVar[event->BaseVarIndex + 2] > 65535 || UserVar[event->BaseVarIndex + 2] < 0) {
            ...
            Plugin_045_write8(SOILMOISTURESENSOR_RESET, SOILMOISTURESENSOR_RESET);
            break;
          }
...
//**************************************************************************/
void Plugin_045_write8(byte reg, byte value)
{
...
//**************************************************************************/
uint8_t Plugin_045_read8(byte reg, bool * is_ok)
{
  uint8_t value;
...
//**************************************************************************/
uint16_t Plugin_045_read16(byte reg)
{
  uint16_t value;
...
//**************************************************************************/
int16_t Plugin_045_readS16(byte reg)
{
  return (int16_t)Plugin_045_read16(reg);
}
...
// Read temperature
//**************************************************************************/
float Plugin_045_readTemperature(uint8_t idx)
{
  return Plugin_045_readS16(SOILMOISTURESENSOR_GET_TEMPERATURE);
}

//**************************************************************************/
// Read light
//**************************************************************************/
float Plugin_045_readLight(uint8_t idx) {
  return Plugin_045_read16(SOILMOISTURESENSOR_GET_LIGHT);
}

//**************************************************************************/
// Read moisture
//**************************************************************************/
unsigned int Plugin_045_readMoisture(uint8_t idx) {
  return Plugin_045_read16(SOILMOISTURESENSOR_GET_CAPACITANCE);
}
