...
* **Discovery Trigger topic**: The discovery can be started by multiple triggers:
...
* **Auto Discovery topic**: The topic that should be used for sending the Discovery data. For Home assistant, this should be set to ``homeassistant/%devclass%/%unique_id%``, the available variables are the same as for the **Controller Publish** settinng, and extended with these:
...
* **Config suffix|online message (/config|online)**: When sending the Discovery data, the topic has to end with ``/config``, by default, but this setting can be changed in he Home assistant configuration. If this is changed, it should be set here (including the ``/`` prefix). The default value is ``/config``, and it can not be an empty value.

  Similarly, the ``online`` message, sent by Home assistant on startup, can be changed. The value can be configured in this field, separated from the ``/config`` setting by a pipe symbol ``|``. If only the ``online`` message is to be changed and uses the default ``/config`` setting, the pipe symbol still has to be inserted too!
