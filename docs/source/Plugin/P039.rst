...
Status ESP32: |P039_status|

Status ESP8266: |P039_status_lb|
...
.. image:: P039_LowTemperatureThreshold.png
  :width: 600

* **Low temperature threshold**: To avoid sending out unrealistic temperatures when the sensor is still initializing and not yet returning valid temperatures, a Low threshold can be configured, that will result in lower values *not* being seen (and sent to controllers) as valid. Default is 0 K (-273.15°C) and the max. allowed input is 1000.0°C
