...
#define PLUGIN_300
#define PLUGIN_ID_300         9
#define PLUGIN_NAME_300       "motor shield - uln2003"
#define PLUGIN_VALUENAME1_300 "motor shield"
#define PLUGIN_300_DOUBLECLICK_MIN_INTERVAL 1000
#define PLUGIN_300_DOUBLECLICK_MAX_INTERVAL 3000
#define PLUGIN_300_LONGPRESS_MIN_INTERVAL 1000
#define PLUGIN_300_LONGPRESS_MAX_INTERVAL 5000
#define PLUGIN_300_DC_DISABLED 0
#define PLUGIN_300_DC_LOW 1
#define PLUGIN_300_DC_HIGH 2
#define PLUGIN_300_DC_BOTH 3
#define PLUGIN_300_LONGPRESS_DISABLED 0
#define PLUGIN_300_LONGPRESS_LOW 1
#define PLUGIN_300_LONGPRESS_HIGH 2
#define PLUGIN_300_LONGPRESS_BOTH 3

boolean Plugin_300(byte function, struct EventStruct *event, String& string)
{
  ...
  switch (function)
  {
    ...
    case PLUGIN_DEVICE_ADD:
      {
        Device[++deviceCount].Number = PLUGIN_ID_300;
    ...
    case PLUGIN_GET_DEVICENAME:
      {
        string = F(PLUGIN_NAME_300);
        break;
      }

    case PLUGIN_GET_DEVICEVALUENAMES:
      {
        strcpy_P(ExtraTaskSettings.TaskDeviceValueNames[0], PSTR(PLUGIN_VALUENAME1_300));
        break;
      }
    ...
    case PLUGIN_WEBFORM_LOAD:
      {
        ...
        const uint32_t key = createKey(PLUGIN_ID_300,Settings.TaskDevicePort[event->TaskIndex]);
        ...
        if (Settings.TaskDevicePluginConfigFloat[event->TaskIndex][1] < PLUGIN_300_DOUBLECLICK_MIN_INTERVAL)
          Settings.TaskDevicePluginConfigFloat[event->TaskIndex][1] = PLUGIN_300_DOUBLECLICK_MIN_INTERVAL;
        ...
        int buttonDCValues[4] = {PLUGIN_300_DC_DISABLED, PLUGIN_300_DC_LOW, PLUGIN_300_DC_HIGH,PLUGIN_300_DC_BOTH};
        addFormSelector(F("Doubleclick event"), F("p009_dc"), 4, buttonDC, buttonDCValues, choiceDC);

        addFormNumericBox(F("Doubleclick max. interval (ms)"), F("p009_dcmaxinterval"), round(Settings.TaskDevicePluginConfigFloat[event->TaskIndex][1]), PLUGIN_300_DOUBLECLICK_MIN_INTERVAL, PLUGIN_300_DOUBLECLICK_MAX_INTERVAL);

        //set minimum value for longpress MIN max speed
        if (Settings.TaskDevicePluginConfigFloat[event->TaskIndex][2] < PLUGIN_300_LONGPRESS_MIN_INTERVAL)
          Settings.TaskDevicePluginConfigFloat[event->TaskIndex][2] = PLUGIN_300_LONGPRESS_MIN_INTERVAL;
        ...
        int buttonLPValues[4] = {PLUGIN_300_LONGPRESS_DISABLED, PLUGIN_300_LONGPRESS_LOW, PLUGIN_300_LONGPRESS_HIGH,PLUGIN_300_LONGPRESS_BOTH};
        addFormSelector(F("Longpress event"), F("p009_lp"), 4, buttonLP, buttonLPValues, choiceLP);

        addFormNumericBox(F("Longpress min. interval (ms)"), F("p009_lpmininterval"), round(Settings.TaskDevicePluginConfigFloat[event->TaskIndex][2]), PLUGIN_300_LONGPRESS_MIN_INTERVAL, PLUGIN_300_LONGPRESS_MAX_INTERVAL);
    ...
    case PLUGIN_WEBFORM_SAVE:
      {
        ...
        for (std::map<uint32_t,portStatusStruct>::iterator it=globalMapPortStatus.begin(); it!=globalMapPortStatus.end(); ++it) {
          if (it->second.previousTask == event->TaskIndex && getPluginFromKey(it->first)==PLUGIN_ID_300) {
    ...
    case PLUGIN_INIT:
      {
        ...
        Plugin_300_Config(Settings.TaskDevicePort[event->TaskIndex], 1);
        ...
        if (Settings.TaskDevicePort[event->TaskIndex] >= 0)
        {
          ...
          newStatus.state = Plugin_300_Read(Settings.TaskDevicePort[event->TaskIndex]);
          ...
          //setPinState(PLUGIN_ID_300, Settings.TaskDevicePort[event->TaskIndex], PIN_MODE_INPUT, switchstate[event->TaskIndex]);
      ...
      case PLUGIN_UNCONDITIONAL_POLL:
        {
          ...
          for (std::map<uint32_t,portStatusStruct>::iterator it=globalMapPortStatus.begin(); it!=globalMapPortStatus.end(); ++it) {
            if ((it->second.monitor || it->second.command || it->second.init) && getPluginFromKey(it->first)==PLUGIN_ID_300) {
              const uint16_t port = getPortFromKey(it->first);
              int8_t state = Plugin_300_Read(port);
    ...
    case PLUGIN_TEN_PER_SECOND:
      {
        const int8_t state = Plugin_300_Read(Settings.TaskDevicePort[event->TaskIndex]);
        /**************************************************************************\
...
#undef FIRED
      ...
      case PLUGIN_EXIT:
      {
        removeTaskFromPort(createKey(PLUGIN_ID_300,Settings.TaskDevicePort[event->TaskIndex]));
        break;
      }
    ...
    case PLUGIN_WRITE:
      {
        ...
        if (command == F("mcpgpio"))
        {
          ...
          if (event->Par1 > 0 && event->Par1 <= 128)
          {
            ...
            const uint32_t key = createKey(PLUGIN_ID_300,event->Par1);
            // WARNING: operator [] creates an entry in the map if key does not exist
            // So the next command should be part of each command:
            tempStatus = globalMapPortStatus[key];

            int8_t currentState = Plugin_300_Read(event->Par1);
            ...
            if (currentState == -1) {
              ...
              tempStatus.state = currentState;
          	  ...
          	  Plugin_300_Write(event->Par1, event->Par2);
          ...
          if (event->Par1 > 0 && event->Par1 <= 128)
          {
            ...
            if (currentState == -1) {
              ...
              tempStatus.mode = PIN_MODE_OUTPUT;
              Plugin_300_Write(event->Par1, tempStatus.state);
          ...
          if (event->Par1 > 0 && event->Par1 <= 128)
          {
            ...
            setPluginTaskTimer(event->Par3 * 1000, PLUGIN_ID_300, event->TaskIndex, event->Par1, !event->Par2);
          ...
          {
            success = true;
            const uint32_t key = createKey(PLUGIN_ID_300,event->Par2); //WARNING: 'status' uses Par2 instead of Par1

            if (existPortStatus(key))
              SendStatusOnlyIfNeeded(event->Source, SEARCH_PIN_STATE, key, dummyString, 0);
            else
           {
             int state = Plugin_300_Read(event->Par2); // report as input
             if (state != -1)
          ...
          {
            success = true;
            const uint32_t key = createKey(PLUGIN_ID_300,event->Par2); //WARNING: 'monitor' uses Par2 instead of Par1
    ...
    case PLUGIN_TIMER_IN:
      {
        Plugin_300_Write(event->Par1, event->Par2);
        //setPinState(PLUGIN_ID_300, event->Par1, PIN_MODE_OUTPUT, event->Par2);
        portStatusStruct tempStatus;
        // WARNING: operator [] creates an entry in the map if key does not exist
        const uint32_t key = createKey(PLUGIN_ID_300,event->Par1);
        tempStatus = globalMapPortStatus[key];
...
// MCP23017 read
//********************************************************************************
int8_t Plugin_300_Read(byte Par1)
{
  int8_t state = -1;
  byte unit = (Par1 - 1) / 16;
...
// MCP23017 write
//********************************************************************************
boolean Plugin_300_Write(byte Par1, byte Par2)
{
  boolean success = false;
  byte portvalue = 0;
  byte unit = (Par1 - 1) / 16;
...
// MCP23017 config
//********************************************************************************
void Plugin_300_Config(byte Par1, byte Par2)
{
  // boolean success = false;
  byte portvalue = 0;
  byte unit = (Par1 - 1) / 16;
