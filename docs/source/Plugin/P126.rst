...
The register outputs are **not** updated on start of the plugin, only when changing a pin state, or using the ``74hcUpdate`` command, the shift registers are updated.
