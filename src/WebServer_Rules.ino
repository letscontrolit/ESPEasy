...
void handle_rules() {
  ...
  html += "let ruleTextFileData = new File([ruleTextNew],'rules' + ruleNumber + '.txt', {type: 'text/plain'});";
  html += "let formData = new FormData();";
  html += "formData.append('file', ruleTextFileData);";
  html += "formData.append('enctype', 'multipart/form-data');";
  html += "let url = '/rules' + ruleNumber + '.txt?callback=' + Date.now();";
  html += "fetch(url).then(res => res.text()).then((ruleTextOld) => {";
    html += "if (ruleTextNew === ruleTextOld) {";
      html += "console.log('nothing to save...');";
    html += "} else {";
      html += "let url = '/upload';";
      html += "fetch(url, {method : 'POST',body: formData}).then(";
        html += "response => response.text()).then(html => {";
        html += "let url = '/rules' + ruleNumber + '.txt?callback=' + Date.now();";
