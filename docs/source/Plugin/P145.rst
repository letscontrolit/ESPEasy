...
This plugin supports some gas sensors for which the resistance depends on a gas concentration. These cheap sensors are tuned for specific gasses although they always respond to multiple gasses. The sensors are sold as MQ-xxx (MQ-3, MQ-135, etc).
...
The plugin can be configured to select one of the predefined sensor types and tune some of the conversion parameters.
...
* The current plugin algorithm assumes the sensor is connected to a supply voltage of 5 Volt and an ESP analog input range of 3.3 Volt. In case your circuit uses a different configuration you need to build the plugin with different values. 
  The load resistance is a device setting. Ohms law is used to determine the sensor's resistance Rsensor.
...
Calibration can be enabled for all sensors. Temperature and Humidity must be provided by external sensors. If enabled the plugin tasks and values for both temperature and Humidity must be configured. 
In case a sensor type does not support compensation the output level will be the same as for compensation turned off.

Calibration
^^^^^^^^^^^

The plugin uses a ration (Rsensor/Rzero) as input to determine the gas level. By changing the value of Rzero the resulting sensor value can be tuned. 
With every measurement the plugin calculates what Rzero should be if the current concentration level equals the reference level. This is the reverse formula for the relation between level and (Rsensor/Rzero) ration.
The plugin will log this value as Rcal. This can be seen on the log output page or on the serial terminal. If a save value can be calculated this will also be shown as a suggestion on device settings page.

The user shall take care that the sensor is well heated and in a clean air (reference for most sensors) for some time. If the Rcal value is stable it can be copied to the Rzero setting on the settings page.
...
The MQ-xxx sensors are cheap sensors for various types of gas. They are primary used to warn for high levels of gas and not for accurate level reporting. For reportinmg these sensors have several drawbacks:
...
Although the serious issues with this kind of sensors they provide a cheap way to do some trend analysis on your environment. They can be used to control ventilation or provide warnings about air quality. 
...
However the MQ-135 keeps the values high much longer that the MHZ-19. Sometimes the MQ-135 detects "gasses" while the MHZ-19 is quiet. 
...
The sensor analog output is connected to an analog input. The digital output often provided on development boards is not used by the plugin. You can use the digital input plugin instead (P001).
...
Please check the expected range for the given sensor and Rload combination. If clipping is expected Rload might be decreased. Other options are to use/change the input divider resistors or apply a 3.3 volt power to the sensor. 
Note that such configurations are not yet supported by the plugin. You have to change some settings in the code and build the plugin yourself.

Device
------

.. image:: P145_DeviceConfiguration.jpg
...
* **R Zero** Reference resistance value for teh sensor. This is the main calibration value, see sensor description.
...
* **Enable temp/hmid compensation** Checkbox to enable temperature and humidity compensation. This requires an external temperature and humidity sensor that deliver the actual data.
