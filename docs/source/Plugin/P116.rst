...
The ST7735 and ST7789 chip family drives color TFT displays in various resolutions, and usually supports 65535 colors (RGB565 coded).

This plugin supports these display models:

* **ST7735** with resolutions 128 x 128, 128 x 160 and 80 x 160 pixels
* **ST7789** with resolutions 240 x 320, 240 x 240 and 240 x 280 pixels
...
* **Text Coordinates in col/row** When checked, the coordinates for the ``txp`` and ``txtfull`` subcommands will be handled in cursor columns & rows, instead of pixels. Column and row are calculated from the current font size and font scaling settings.
...
* *Color name* Some named colors are recognized: White, Black, Red, Green, Blue, Yellow, Orange, Navy, Darkgreen, Cyan, Darkcyan, Maroon, Purple, Olive, Magenta, Lightgrey, Darkgrey, Pink and Greenyellow.
...
If the Foreground and Background colors are the same, f.e. when saved empty, the default White (#FFFF) foreground and Black (#0) background colors will be set.

* **Wake display on receiving text** When checked, the display will be enabled once any content is written to the screen, either triggered by the Interval, or from a command.
...
The total combination of lines * input length can not exceed 1024 characters (sized dynamically), as there is limited storage per task available for these settings. An error message will be shown after (trying to) save the settings, **any excess content will be discarded!**
