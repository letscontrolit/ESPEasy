//#######################################################################################################
//########################### Controller Plugin 012: volkszaehler.org HTTP ##################################
//#######################################################################################################

#define CPLUGIN_012
#define CPLUGIN_ID_012         12
#define CPLUGIN_NAME_012       "volkszaehler.org HTTP"
    ...
    case CPLUGIN_PROTOCOL_ADD:
      {
        ...
        Protocol[protocolCount].usesUUID = true;
    ...
    case CPLUGIN_PROTOCOL_SEND:
      {
        ...
        for (byte x = 0; x < valueCount; x++)
        {
          if (event->sensorType == SENSOR_TYPE_LONG)
            VolkszaehlerHTTPSend(event, 0, 0, (unsigned long)UserVar[event->BaseVarIndex] + ((unsigned long)UserVar[event->BaseVarIndex + 1] << 16));
          else
            VolkszaehlerHTTPSend(event, x, UserVar[event->BaseVarIndex + x], 0);
          if (valueCount > 1)
          {
...
//********************************************************************************
// volkszaehler.org HTTP
//********************************************************************************
boolean VolkszaehlerHTTPSend(struct EventStruct *event, byte varIndex, float value, unsigned long longValue)
{
  ...
  url.replace(F("%uuid%"), ControllerSettings.UUID);
