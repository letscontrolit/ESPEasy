...
**Output values** When the PMS5003ST **Sensor model** is selected, extra output values are available. To choose which values are made available in the task, this combobox provides the available options

Available options:

.. image:: P053_OutputValuesOptions.png

*Particles: pm1.0, pm2.5, pm10* The default output values, available in all supported sensors.
...
*Particles: pm2.5; Other: Temp, Humi, HCHO (PMS5003ST)* Output values, providing some measurements only supported by the PMS5003ST model.
...
**Events for non-output values** When enabled, and the PMS5003ST model is selected, the **Output values** that are not directly available, will be sent via events. The names of these events are:

When *Particles: pm1.0, pm2.5, pm10* is selected: ``<Taskname>#Temp``, ``<Taskname>#Humi`` and ``<Taskname>#HCHO``, with the measured value available in ``%eventvalue1%`` (or it's alias ``%eventvalue%``).

When *Particles: pm2.5; Other: Temp, Humi, HCHO (PMS5003ST)* is selected: ``<Taskname>#pm1.0`` and ``<Taskname>#pm10``.
