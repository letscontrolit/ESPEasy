...
For details on the Modbus protocol, please refer to https://www.modbustools.com/modbus.html or other official Modbus documentation.
...
The plugin can use a cache to store a limited amount of consequitive register values. The stored values of these registers are available through commands. The cache is read using a single Modbus request, which is more efficient than reading each register individually. Note that the maximum size of the cache may be limited by the actual Modbus device.
...
Modbus RTU requires a serial interface and a serial to RS485 converter like the board shown below. Due to memory constraints for ESP8266, the plugin is only available for ESP32 based devices.

.. image:: P183_rs485_converter.jpeg
  :width: 400
