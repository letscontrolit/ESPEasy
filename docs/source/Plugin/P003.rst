...
:Ignore Delta = 0:
  When enabled doesn't generate events or send data to Controllers if the Delta (Count) is 0 since the last time the data is read. This also suppresses then initial Read action when the plugin is started!
