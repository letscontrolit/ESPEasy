...
const __FlashStringHelper* Plugin_188_output_mapping_name(uint8_t value_nr, bool displayString) {
  const __FlashStringHelper *strings[] {
    ...
    F("abs(AIN1 - AIN0) (Difference)"),     F("V_abs_AIN1_AIN0"), // 8
    F("abs(AIN3 - AIN2) (Difference)"),     F("V_abs_AIN3_AIN2"), // 9
    F("abs(AIN5 - AIN4) (Difference)"),     F("V_abs_AIN5_AIN4"), // 10
    F("abs(AIN7 - AIN6) (Difference)"),     F("V_abs_AIN7_AIN6"), // 11

# if P188_FEATURE_RESISTOR_MEASUREMENT
    F("AIN1, AIN0 Resistor Measurement"),   F("R_AIN1_AIN0"),     // 12
    F("AIN3, AIN2 Resistor Measurement"),   F("R_AIN3_AIN2"),     // 13
    F("AIN5, AIN4 Resistor Measurement"),   F("R_AIN5_AIN4"),     // 14
    F("AIN7, AIN6 Resistor Measurement"),   F("R_AIN7_AIN6")      // 15
...
bool P188_data_struct::sample(void) // called each 100ms, runtime: ~800us
{
  ...
  success = TLA2528_write_single_reg(P188_config.i2cAddress, TLA2528_REG_AUTO_SEQ_CH_SEL, (0x03 << _sample_cnt)); // configure auto conversion sequence
                                                                                                                  // for channels 0/1 or 2/3 or 4/5 or 6/7
