        ...
        if (cmdCode.equalsIgnoreCase(F("IRSEND")) && Plugin_035_irSender != 0)
        {
          ...
          if (IrType.equalsIgnoreCase(F("RAW"))) {
            ...
            if (GetArgv(command, TmpStr1, 3)){ IrCode = strtoul(TmpStr1, NULL, 16); //(long) TmpStr1
                                              IrCode16 = strtoul(TmpStr1, NULL, 16);
            ...
            //if (IrType.equalsIgnoreCase(F("PANASONIC"))) Plugin_035_irSender->sendPanasonic(IrCode); // not sure ig this works
            ...
            if (IrType.equalsIgnoreCase(F("RAWTEST"))) parseStringAndSendRaw(ircodestr);
          }
