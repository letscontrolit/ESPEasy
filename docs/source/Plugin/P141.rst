...
The available options rotate the display content in a clock-wise direction.
...
This monochrome display only supports colors **white** and **black**
...
* **Linespacing** When using a font scale > 1 the whitespace between the lines can be quite big. To be able to fine-tune that, this option is available, allowing 0 .. 14 pixels fixed linespacing, or use the default 'Auto' setting that calculates the linespacing based on font height and scale. When using 0 or 1 pixels, possibly the characters 'hanging' below the baseline, like g, j, p and y may get partially overwritten by the next line!
