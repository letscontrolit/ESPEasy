...
const __FlashStringHelper* Plugin_188_output_mapping_name(uint8_t value_nr, bool displayString) {
  const __FlashStringHelper *strings[] {
    ...
    F("ABS|AIN1 - AIN0| (Differential)"),    F("V_|AIN10|"),       // 8
    F("ABS|AIN3 - AIN2| (Differential)"),    F("V_|AIN32|"),       // 9
    F("ABS|AIN5 - AIN4| (Differential)"),    F("V_|AIN54|"),       // 10
    F("ABS|AIN7 - AIN6| (Differential)"),    F("V_|AIN76|"),       // 11
...
bool P188_data_struct::init(struct EventStruct *event) {
  if (event != nullptr) {
    ...
    if (initialized) {
      ...
      // uint8_t retVal;
      // retVal = P188_data_struct::TLA2528_read_single_reg(P188_config.i2cAddress, TLA2528_REG_SYSTEM_STATUS);
      // addLog(LOG_LEVEL_INFO, strformat(F("P188: read value: %d"), retVal));
      // retVal = P188_data_struct::TLA2528_read_single_reg(P188_config.i2cAddress, TLA2528_REG_GENERAL_CFG);
      // addLog(LOG_LEVEL_INFO, strformat(F("P188: read value: %d"), retVal));
      ...
      _sample_cnt = 0;
      while (TLA2528_BIT_GC_RST & P188_data_struct::TLA2528_read_single_reg(P188_config.i2cAddress, TLA2528_REG_GENERAL_CFG))  {
      ...
      _sample_cnt = 0;
      while (TLA2528_BIT_GC_CAL & P188_data_struct::TLA2528_read_single_reg(P188_config.i2cAddress, TLA2528_REG_GENERAL_CFG)) {
        delayMicroseconds(500);
        if (_sample_cnt > 9) {
          return(false);
        }      
      ...
      initialized = initialized && P188_data_struct::TLA2528_write_single_reg(P188_config.i2cAddress, TLA2528_REG_DATA_CFG, TLA2528_BIT_DC_APPEND_STATUS);  // append channel ID
    }
    _sample_cnt = 0;
  }
  return(initialized);
...
bool P188_data_struct::sample(void) // call each 100ms, runtime: ~800us
{
  /* each call of P188_data_struct::sample() converts 2 ADC inputs */
  ...
  if ((_sample_cnt & 0x01) || (_sample_cnt > 6))
    _sample_cnt = 0;

  // addLog(LOG_LEVEL_INFO, strformat(F("P188: sample %d"), _sample_cnt));
  ...
  if (success){
    /* be aware that raw data contains the channel nummer in bits[0:3] */
...
bool P188_data_struct::read_raw(struct EventStruct *event, float& value, uint8_t ch_num) const {
  ...
  value = raw_samples[ch_num] >> 4; // remove channel number
  return(true);
}
